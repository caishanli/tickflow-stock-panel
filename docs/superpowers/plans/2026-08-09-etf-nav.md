...
- **历史不补**：akshare `fund_etf_fund_daily_em` 只有当前快照（无逐日历史），回补历史只会把今日净值写成过去每一天（假数据）。`etf_nav` 只同步「最新缺失交易日」——`_missing_etf_nav_days()` 最多返回一个候选日，`scan_missing_partitions` 的 `etf_nav` 键与 `backfill_to_now` 都用它（不用 `_missing_days_in` 全日历）。
...
在 `scan_missing_partitions()` 的返回 dict（`mootdx_service.py:731-737`）加一行（注意 `etf_nav` 用 `_missing_etf_nav_days`，**不是** `_missing_days_in`——后者会铺开 4/1 以来的全部历史，写入假净值分区）：
...
- Modify: `backend/tests/quant/test_engine_order_rules.py`（`_daily_mgr` fake 加 `.client`，更新 2 个 get_extras 断言为真实净值）
...
- [ ] **Step 6.5: 更新 `test_engine_order_rules.py` 的 get_extras 断言**

Task 2 改动后 `get_extras('unit_net_value')` 不再用 close 近似，而是走
`manager.client.get_etf_nav`。现有 `_daily_mgr()` fake 无 `.client` 属性，
且旧断言 `iloc[-1] == 1.2`（close 近似值）失效。改为：
...
```bash
git add backend/app/services/stockdata/sources.py backend/app/services/stockdata/handlers.py backend/app/quant/datasource/network_client.py backend/app/quant/jqcompat.py backend/app/quant/jqengine/engine/jq/api.py backend/tests/quant/test_etf_nav_client.py backend/tests/quant/test_engine_order_rules.py
