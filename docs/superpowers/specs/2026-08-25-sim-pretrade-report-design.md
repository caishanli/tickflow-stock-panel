# 模拟盘预买卖报告（d092ad90 克隆 + 11:20/13:01 预报告）设计
...
2. 新账户使用策略副本，在每天 **11:20** 与 **13:01** 各发一条**预买卖报告**：
   - 预计买入（目标集 − 当前持仓，按排名顺序）；
...
1. **注册定时任务**（`initialize` 内）：
   ...
   ```python
   run_daily(pre_trade_report, time='11:20')  # 午间预报告
...
| 场景 | 处理 |
|---|---|
| 11:20 量比外推 | `get_volume_ratio` 已含午休扣减（`hour>=13` 减 90）；上午以 elapsed=110 分钟外推，口径一致 |
| 11:20 bar 触发 | `_daily_due('11:20', bar_dt)` 在上午收盘 bar（11:20）满足 `time>=HH:MM`，正常触发 |
