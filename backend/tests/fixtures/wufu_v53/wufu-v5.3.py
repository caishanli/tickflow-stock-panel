...
- A1 盈利保护止损: {'启用' if g.enable_profit_protect else '禁用'} (浮盈≥{g.profit_protect_trigger:.0%}后止损上移至成本价)
=== v5.3 其他改进 ===
...
def minute_level_stop_loss(context):
    ...
    for security in list(context.portfolio.positions.keys()):
        ...
        if current_price <= cost_price * stop_threshold:
            ...
            stop_label = "保本线" if stop_threshold >= 1.0 else f"成本×{g.fixedStopLossThreshold:.0%}"
