...
#行业ETF：
    ...
    g.holdings_num = 2
    g.cross_slot1_floor = 0.3
    g.cross_slot1_retain_ratio = 0.85
    g.cross_adaptive = True
    g.cross_weight_cap = 0.85
    g.target_weights = [0.5, 0.5]
...
def execute_buy_trades(context):
    ...
    for i in range(num_etfs_to_buy):
        ...
        # 自适应加权分配: 新买入槽位目标市值 = 总资产 × 槽位权重(target_weights);
        # 单持仓模式 weights=[1.0] -> 全仓。最后一笔用剩余现金消化余量。
...
def select_cross_asset_dual(current_holdings, filtered_list, score_key, log_buffer):
    ...
    log_buffer.append(f"【双持仓选择】slot1下限={slot1_floor} 自适应={adapt}")
    ...
    if held_scored:
        ...
        if best_held.get(score_key, float('-inf')) >= top.get(score_key, float('-inf')) * 0.9:
            slot0 = best_held
            log_buffer.append(f"【保留 slot0】{best_held['etf_name']}({best_held['etf']}) 得分{best_held.get(score_key,0):.4f} ≥ 第一×0.9")
    ...
    if other_class:
        ...
        for m in other_class:
            if m['etf'] in current_holdings and m.get(score_key, float('-inf')) >= other_top_score * slot1_retain:
                slot1 = m
                log_buffer.append(f"【保留 slot1】{m['etf_name']}({m['etf']}) 得分{m.get(score_key,0):.4f}")
                break
    if slot1 is None:
        g.target_weights = [1.0]
        log_buffer.append(f"【双持仓选择】slot1 空缺 → 退化为单持仓: {slot0['etf_name']}({slot0['etf']})")
        return [slot0]
    ...
    log_buffer.append(f"【双持仓选择】权重 {w1}/{w2}")
    log_buffer.append("【最终目标】共2只ETF：")
    for i, item in enumerate([slot0, slot1]):
        cls = '全球/海外' if _is_global(item['etf']) else '大A/港股'
        log_buffer.append(f"  {i+1}. {item['etf_name']}({item['etf']}) [{cls}] {score_key}: {item.get(score_key,0):.4f}")
    return [slot0, slot1]
