# ============================================================
# 【五福闹新春】v5.4 — 70978ed5 PTrade 原生移植版
# 原版（聚宽 JoinQuant）：backend/tests/fixtures/70978ed5_ptrade/70978ed5.py
...
#   - 调度：晨间 → before_trading_start(context, data)；盘中 09:40 + 13:10×3（同点，按注册顺序
#           afternoon_routine→sell_routine→buy_routine）→ run_daily(context, func, time)；
...
#   - 持仓：get_positions() 返回 {code: Position}（amount / enable_amount / cost_basis / last_sale_price），
#           键可能为 .XSHG/.XSHE（官方文档确认），策略用 _positions() 归一为 .SS/.SZ 再与池子代码比较
#   - 现金/总资产：context.portfolio.cash / .portfolio_value（PTrade 无 get_cash）
#   - 动态 ETF 池：get_etf_list()（本地引擎）；真机回退 get_market_list()/get_market_detail() 枚举全市场基金
#     （_get_all_fund_codes）；成交额查询按 200 只分块（_get_money_avg_series，
#     money_corrected 真机无此字段自动回退官方 money）
#   - 日志：log.debug/info/warning/error（无 log.warn）；set_benchmark/set_commission/set_slippage 仅回测可用，
...
def _positions():
    ...
    try:
        for code, pos in (get_positions() or {}).items():
            if not code:
                continue
            # 真机 get_positions() 键可能为 .XSHG/.XSHE（官方文档确认），归一为 .SS/.SZ
            # （拼接避免在策略源码中出现聚宽后缀字面量）
            code = str(code).replace('.X' + 'SHG', '.SS').replace('.X' + 'SHE', '.SZ')
...
def _get_money_avg_series(codes, count, context, field='money'):
    ...
    避免对上千只标的单次 get_history 查询导致回测挂起。field='money_corrected'：
    本地引擎返回修正后的元成交额（对齐聚宽 get_daily_money_cached 口径）；真机无该字段，
    经 _resolve_money_field 自动回退官方 'money'。当日全市场结果缓存复用。"""
...
def _get_money_daily_totals(codes, context, field='money'):
    ...
    try:
        ...
        for i in range(0, len(codes), CHUNK):
            ...
            df = _wide(get_history(3, '1d', field, security_list=chunk), field)
            if df is None or df.empty:
                continue
            df = df.fillna(0.0)
...
def _ensure_fund_universe(context):
    """缓存全市场基金代码表 g._fund_universe（列表），失败则空表"""
    if getattr(g, '_fund_universe', None) is None:
        codes = _get_all_fund_codes(context)
        g._fund_universe = codes if codes else []
...
def initialize(context):
    ...
    log.info("【五福闹新春】v5.4（胜率优化版）(PTrade 移植版)！")
...
""" % (
        ...
        g.holdings_num, g.defensive_etf, g.min_money,
    ))
...
def afternoon_routine(context):
    ...
    try:
        set_universe([*g.merged_etf_pool, g.defensive_etf])
    except Exception as e:
        log.warning('set_universe 更新失败: %s' % e)
    # 原聚宽 preload_minute_for_pool 预热分钟缓存：PTrade 由引擎按需回源，无需显式预热
...
def calculate_global_etf_threshold(context):
    ...
    try:
        # 缓存全市场 ETF 列表（仅首次获取，PTrade 用 get_etf_list/get_market_detail 枚举）
        if not hasattr(g, '_cached_etf_universe') or g._cached_etf_universe is None:
            g._cached_etf_universe = _ensure_fund_universe(context)
            log.info("全市场ETF总数: %d只 (已缓存)" % len(g._cached_etf_universe))
        ...
        if len(trade_days) < 3:
            ...
            g.avg_etf_money_threshold = 10000000
            return
        # 直接从 get_history 分块读取成交额（对齐聚宽 get_daily_money_cached 口径，200 只/块）
        ...
        if not _is_real_ptrade():
            daily_totals = _get_money_daily_totals(etf_list, context, field='money_corrected')
            if daily_totals is not None:
                for day, (money, count) in daily_totals.items():
                    log.info("  %s 全市场ETF总成交额: %.2f亿元 (%d只ETF有成交)" % (day, money / 1e8, count))
        if daily_totals is not None and len(daily_totals) < 3:
            log.warning("仅有%d个有效交易日，使用保守阈值1000万" % len(daily_totals))
            g.avg_etf_money_threshold = 10000000
            return
        ...
        log.info("【全局阈值更新完成】近%d日全市场ETF日均总成交额=%.2f亿元，阈值=%.0f万元(%s元)" % (
            len(daily_totals) if daily_totals is not None else 3,
...
def filter_global_pool_by_volume(context):
    ...
    TRADE_DAYS_COUNT = 3
    try:
        # 直接从 get_history 分块读取成交额（对齐聚宽 get_daily_money_cached 口径）
        avg_daily_money = _get_money_avg_series(g.global_etf_pool, TRADE_DAYS_COUNT, context, field='money_corrected')
        if avg_daily_money.empty:
...
def update_sector_pool(context):
    ...
    try:
        etf_list = _ensure_fund_universe(context)
        if not etf_list:
        ...
        g.etf_names_dict = {}
        try:
            nd = get_stock_name(etf_list) or {}
            for c in etf_list:
                n = nd.get(c)
                if n:
                    g.etf_names_dict[c] = n
        except Exception:
            pass
    except Exception as e:
    ...
    log.info("【动态池更新】全市场ETF总数: %d只" % len(etf_list))
    normal_etfs = []
    special_etfs = []
    special_group_map = {}
    excluded_count = 0
    ...
    normal_qualified, _normal_filtered_out = filter_by_liquidity(normal_etfs, "普通组")
    special_qualified, _special_filtered_out = filter_by_liquidity(special_etfs, "特别组")
    ...
    final_pool_info = []
    for _industry_key, items in normal_industry_groups.items():
        sorted_items = sorted(items, key=lambda x: x['money'], reverse=True)
        final_pool_info.append(sorted_items[0])
    for _group_key, items in special_industry_groups.items():
        sorted_items = sorted(items, key=lambda x: x['money'], reverse=True)
        final_pool_info.append(sorted_items[0])
...
def check_a_share_weak_period(context):
    ...
    for name, code in indexes.items():
        df = get_history(data_lookback + 1, '1d', 'close', security_list=code, fq='pre')
...
def apply_filters(metrics_list):
    ...
    for _name, condition, is_enabled in steps:
        if is_enabled:
...
def get_final_ranked_etfs(context):
    ...
    for etf in etf_set:
        try:
            ...
            if is_temporarily_suspended(etf, context):
                log.info("⏭️ %s %s 临时停牌/分钟数据缺失，跳过动量计算" % (etf, _security_name(etf)))
                continue
            ...
            skipped_no_minute.append((etf, _security_name(etf), str(e)))
            log.warning("⚠️ %s %s 分钟数据获取失败，跳过: %s" % (etf, _security_name(etf), e))
            continue
        if metrics:
    ...
    log_buffer.append("其中存在于候选池中的持仓ETF：%s" % [item['etf'] for item in retained])
    ...
    if hold_buffer < 1.0 and len(retained) < g.holdings_num:
        ...
        for etf in current_holdings:
            ...
            if holding_score != float('-inf') and holding_score >= pool_threshold:
                ...
                log_buffer.append("【持仓宽容】%s(%s) 得分%.4f ≥ 候选池门槛×%.2f=%.4f，保留持仓" % (
                    holding['etf_name'], etf, holding_score, hold_buffer, pool_threshold))
    if len(retained) >= g.holdings_num:
        ...
        log_buffer.append("保留的持仓ETF数量(%d)超过目标持仓数(%d)，将从保留的ETF中按动量得分取前%d只作为最终目标。" % (
        ...
        log_buffer.append("保留持仓ETF %d只，还需补充%d只。" % (len(retained), need))
        if retained:
            log_buffer.append("保留的ETF（按原有顺序）：")
            for item in retained:
                log_buffer.append("  %s(%s)" % (item['etf_name'], item['etf']))
        if additional:
            log_buffer.append("补充的ETF（按动量得分排序）：")
            for i, item in enumerate(additional):
                log_buffer.append("  %d. %s(%s) %s: %.4f" % (i + 1, item['etf_name'], item['etf'], score_key, item.get(score_key, 0)))
    log_buffer.append("【最终目标】共%d只ETF：" % len(final_result))
    for i, item in enumerate(final_result):
        log_buffer.append("  %d. %s(%s)" % (i + 1, item['etf_name'], item['etf']))
    log_buffer.append("==================================================")
    full_log = "\n".join(log_buffer)
    log.info(full_log)
    return final_result
...
def execute_sell_trades(context):
    ...
    if ranked_etfs:
        ...
        if check_defensive_etf_available(context):
            ...
            etf_name = _security_name(g.defensive_etf)
...
def is_temporarily_suspended(security, context, minute_count=10):
    ...
    """
    try:
        # 获取最近N分钟的分钟线数据（get_price 走引擎分钟缓存/按需回源）
        minute_data = get_price(security, end_date=_current_dt(context), count=minute_count,
        ...
        if minute_data is None or minute_data.empty:
            log.warning("⚠️ %s %s 分钟数据缺失（%s 最近%d分钟无数据），按临时停牌处理，若为盘中取数异常请排查数据源" % (
                security, _security_name(security), _current_dt(context), minute_count))
            return True
        ...
        # 最近N分钟成交量都为0，视为临时停牌
        if np.all(vals == 0):
            log.info("🔇 %s %s 最近%d分钟无成交，判定临时停牌" % (security, _security_name(security), minute_count))
            return True
        return False
    except Exception as e:
...
def smart_order_target_value(security, target_value, context):
    ...
    if target_value > 0:
        ...
        target_amount = 0
    cur_pos = _positions().get(security, None)
