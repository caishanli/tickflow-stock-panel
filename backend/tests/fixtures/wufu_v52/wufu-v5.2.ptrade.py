# -*- coding: utf-8 -*-
# ============================================================
# 【五福闹新春】v5.2 — PTrade 移植版
# 原版（聚宽 JoinQuant）：backend/tests/fixtures/wufu_v52/wufu-v5.2.py
...
def initialize(context):
    ...
    run_daily(context, afternoon_routine, time='13:00')          # 动量计算与排序（需早于卖出时间）
    run_daily(context, sell_routine, time='13:05')               # 卖出流水线（需早于买入时间）
    run_daily(context, buy_routine, time='13:15')                # 买入流水线
    ...
    log.info("【五福闹新春】v5.2 (PTrade 移植版)！")
...
""" % (
        ...
        g.weak_period_ma_lookback, g.weak_period_ma_lookback, g.weak_period_ma_lookback,
        g.max_weak_days,
...
def check_a_share_weak_period(context):
    ...
    for name, code in indexes.items():
        df = get_history(g.weak_period_ma_lookback + 1, '1d', 'close', security_list=code)
        closes = _as_series_values(df)
        if closes is None or len(closes) < g.weak_period_ma_lookback:
    ...
    log.info("📊 【走弱期判断】低于MA%d: %d/4, 站上MA%d: %d/4" % (
        g.weak_period_ma_lookback, below_count, g.weak_period_ma_lookback, above_count))
