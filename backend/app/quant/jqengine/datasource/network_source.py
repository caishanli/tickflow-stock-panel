...
class NetworkSource(DataSource):
    ...
    def _fetch_daily(self, code, start, end, fq: str | None = None) -> pd.DataFrame:
        out = self.client.get_price(code, start_date=start, end_date=end,
                                    frequency="daily", fq=fq)
        df = out.get(code)
        if df is None or df.empty:
            raise DataSourceError(f"网络无日线数据: {code}")
        return df

    def get_daily(self, code, start, end, fq: str | None = None):
        return self._fetch_daily(code, start, end, fq=fq)

    def get_daily_batch(self, codes, start, end, fq: str | None = None):
