"""最小报价单位（tick）——成交价取整的唯一实现。

回测撮合（``jqcompat._patch_matcher_tick_rounding``）、本地引擎
（``core.execution.execute_order``，经 jq/ptrade 两 API 面）、止损撮合
（``simulate.matcher``）三处成交价取整必须走同一实现，否则同策略回测
vs 补跑逐笔价差半个 tick、净值系统性漂移。
...
注意：本判定只用于成交价取整。``core.instruments.is_etf`` 是印花税免征判定，
...
def tick_size(code: str | None) -> float:
    """给定代码（``510300.XSHG``，纯数字亦可）返回最小报价单位。"""
...
def round_to_tick(price: float, code: str | None) -> float:
    """成交价按标的 tick 取整（step quantize，全 core 调用方的唯一实现）。
