...
def run_daily(func, time=None, *args, **kwargs):
    ...
    if time_rule == "before_trading":
        ...
        try:
            ...
            hm = (9, 31)
    # 交易时段内的 bar 时刻映射：聚宽以「起始时刻」标注 bar，本地以「收盘时刻」
    # 标注且午后首根为 13:01（无 13:00 bar）。聚宽 run_daily('13:00') 在开盘瞬间
    # 触发，其 current_data.last_price = 11:30 收盘价（午后尚无成交），对应本地
    # 11:30 bar 的 close。故将 (13,0) 映射到 (11,30)，否则会落到 13:01 bar 拿到
    # 13:01 收盘价而错开一分钟。
    if hm == (13, 0):
        hm = (11, 30)
