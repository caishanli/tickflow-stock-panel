...
// 拼接十六进制 alpha（如 '26'）会得到非法颜色，导致 echarts addColorStop 抛错。
// 这里统一包成合法 CSS：solid = hsl(...)，alpha 用现代 / 语法。
...
export function EquityChart({ equity }: { equity: any[] }) {
  const option = useMemo(() => {
    ...
    return {
      ...
      series: [
        { name: '策略', type: 'line', data: s, symbol: 'none', lineStyle: { color: hslColor(accent), width: 2 },
          areaStyle: { color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: hslColor(accent, 0.15) }, { offset: 1, color: hslColor(accent, 0.01) }] } } },
