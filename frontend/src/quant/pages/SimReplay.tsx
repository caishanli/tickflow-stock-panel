...
export function SimReplay() {
  ...
  // 离线回放共享回测轮询节奏（回测 1–2s）
  const { data: status } = useQuery({
    queryKey: ['quant', 'replay', runId, 'status'], queryFn: () => api.getBacktestStatus(runId!),
    enabled: !!runId, refetchInterval: 1500,
  })
  const { data: equity } = useQuery({ queryKey: ['quant', 'replay', runId, 'equity'], queryFn: () => api.getBacktestEquity(runId!), enabled: !!runId, refetchInterval: 1500 })
  const { data: trades } = useQuery({ queryKey: ['quant', 'replay', runId, 'trades'], queryFn: () => api.getBacktestTrades(runId!), enabled: !!runId, refetchInterval: 1500 })
  const { data: logs } = useQuery({ queryKey: ['quant', 'replay', runId, 'logs'], queryFn: () => api.getBacktestLogs(runId!), enabled: !!runId, refetchInterval: 1500 })

  return (
    <div className="flex flex-col h-full">
      <PageHeader title="离线回放" subtitle="RQAlpha · 聚宽式策略" />
      <div className="flex-1 grid grid-cols-[320px_1fr] overflow-hidden">
        ...
        <aside className="border-r border-border p-3 space-y-2 overflow-auto">
          <div className="text-xs text-muted">选择回放策略</div>
          {(strategies ?? []).map((s: any) => (
            <div key={s.id} className="flex items-center justify-between rounded-card border border-border bg-surface px-3 h-10 text-xs">
              ...
              <div className="flex gap-2 shrink-0">
                <button onClick={() => setForm(f => ({ ...f, strategyId: s.id }))} className={form.strategyId === s.id ? 'text-accent' : 'text-muted'}>选</button>
        ...
        <section className="p-4 space-y-4 overflow-auto">
          <div className="grid grid-cols-2 gap-3 max-w-2xl">
            ...
            <button onClick={() => runMut.mutate()} disabled={!form.strategyId || !form.start || !form.end || runMut.isPending}
              className="h-9 rounded-lg bg-accent text-white text-xs disabled:opacity-50">{runMut.isPending ? '回放中…' : '运行回放'}</button>
          </div>
          {runId && status ? (
            <BacktestResult status={status} equity={equity} trades={trades} logs={logs} />
          ) : (
            <EmptyState title="尚未运行" hint="选择聚宽策略与区间后点击运行" />
          )}
        </section>
      </div>
    </div>
